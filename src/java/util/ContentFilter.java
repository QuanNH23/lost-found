...
public class ContentFilter {
    ...
    private static boolean persistToFile(String filename, Set<String> data, String appRealPath) {
        try {
            ...
            // Also persist to source path if exists
            if (appRealPath != null) {
                String sourceRoot = appRealPath.replace("build\\web", "src\\java").replace("build/web", "src/java");
                Path srcPath = Paths.get(sourceRoot);
                if (Files.exists(srcPath)) {
                    Path sourceFile = srcPath.resolve(filename);
